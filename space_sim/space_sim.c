...
int main(int argc, char *argv[])
{
    ...
    if (!SDL_CreateWindowAndRenderer("Space Sim", SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_RESIZABLE, &window,
                                     &renderer))
    {
    ...
    const char *paths[BODY_COUNT] = {// order must match with BodyTex enum
                                     "../imgs/assets/sun.png",   "../imgs/assets/mercury.png",
                                     "../imgs/assets/venus.png", "../imgs/assets/earth.png",
                                     "../imgs/assets/moon.png",  "../imgs/assets/mars.png"};
    ...
    Body bodies[BODY_COUNT] = {[SUN] = {.name = "Sun",
                                        .orbit_speed = 0.0f,
                                        .orbit_radius = 0.0f,
                                        .angle = 0,
                                        .texture = tex[SUN],
    ...
    float planetCenterPoint[2] = {(bodies[SUN].rect.x + bodies[SUN].rect.w / 2.0f),
    ...
    while (running)
    {
        ...
        for (int i = 0; i < BODY_COUNT; i++)
        {
            ...
            SDL_FRect draw_rect = {sX, sY,
                                   bodies[i].rect.w * main_cam.zoom, // Scale size by zoom
                                   bodies[i].rect.h * main_cam.zoom};

            SDL_RenderTextureRotated(renderer, bodies[i].texture, NULL, &draw_rect, bodies[i].angle * 0.5f, NULL,
                                     SDL_FLIP_NONE);
        }
